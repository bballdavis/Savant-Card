...
describe("statistics refresh", () => {
  it("loads breaker statistics in parallel and applies them together", async () => {
    ...
    const refresh = (card as any).loadStatistics();
    first.resolve(stat("sensor.a_power", 10));
    await Promise.resolve();
    await Promise.resolve();

    expect((card as any).stats.has("sensor.a_power")).toBe(false);
